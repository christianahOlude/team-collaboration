...
import Banner from './Banner.jsx'; // Imports the Configuration Banner
import WelcomeBar from './WelcomeBar.jsx'; // Imports the Welcome Bar

// --- Icon Definitions (Shared between components) ---
// Using bolder icons to match the overall design feel
const Icon = ({ children, className = "w-6 h-6", strokeWidth = "1.5" }) => (
    <svg className={className} fill="none" stroke="currentColor" strokeWidth={strokeWidth} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        {children}
    </svg>
);
const LearnersIcon = (props) => (<Icon {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></Icon>);
const RecentVerificationIcon = (props) => (<Icon {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.636-4.364a9 9 0 11-12.728 0m12.728 0L21 7" /></Icon>);
const CourseIcon = (props) => (<Icon {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5s3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18s-3.332.477-4.5 1.253" /></Icon>);
const InstituteIcon = (props) => (<Icon {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m-1 4h1m-1 4h1" /></Icon>);
const CalendarIcon = (props) => (<Icon {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h.01M12 11h.01M15 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></Icon>);


// --- 1. KPI Card Component (Matching the image's layout) ---
const DashboardCard = ({ title, value, icon: Icon, color }) => (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between h-32">
        <div className="flex items-center justify-between">
            ...
            {/* Icon (Colored and sized to match) */}
            <div className={`p-2 rounded-lg ${color.bg} bg-opacity-20`}>
...
// --- 2. Calendar Component (Sidebar) ---
const ActivityCalendar = () => {
    // Mock data for the calendar view
    const days = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
    const mockDates = Array.from({ length: 30 }, (_, i) => i + 1);
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
            ...
            <div className="grid grid-cols-7 gap-1 text-center text-xs">
                ...
                {/* Filler for start of month alignment */}
                {Array.from({ length: 2 }).map((_, i) => <div key={`fill-${i}`}></div>)}

                {mockDates.map(date => (
                    <div
                        key={date}
                        className={`p-1 rounded-full w-7 h-7 flex items-center justify-center cursor-pointer 
                            ${date === 15 ? 'bg-indigo-600 text-white font-bold shadow-md' : 'text-gray-700 hover:bg-gray-100'}
                            ${date === 17 || date === 24 ? 'bg-indigo-100 text-indigo-800 font-semibold' : ''}
                        `}
                    >
                        {date}
                    </div>
            ...
            <p className="text-xs text-gray-500 pt-2 border-t mt-4">
                <span className="font-semibold text-indigo-700">15th Feb:</span> Pending Assessments
            </p>
        </div>
    );
}

// --- 3. Course List Component (Matching the list layout) ---
const RecentCourses = () => {
    // Mock data for the course list
    const courses = [
        { name: 'Introduction to UI Design', instructor: 'Wema Alabi', date: '01 Jan 2023', profile: 'WA' },
        { name: 'Advanced Python for ML', instructor: 'Pravin Reddy', date: '01 Jan 2023', profile: 'PR' },
        { name: 'Financial Modelling', instructor: 'Alex Oluchi', date: '01 Jan 2023', profile: 'AO' },
        { name: 'Cyber Security Basics', instructor: 'Femi Johnson', date: '01 Jan 2023', profile: 'FJ' },
        { name: 'Cloud Computing Fundamentals', instructor: 'Sara Khan', date: '01 Jan 2023', profile: 'SK' },
    ];
                ...
                {courses.map((course, index) => (
                    <div key={index} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <p className="text-sm font-medium text-gray-800 truncate max-w-[120px]">{course.name}</p>
                                <p className="text-xs text-gray-500">Instructor: {course.instructor}</p>
                            </div>
                        </div>
                        <span className="text-xs font-medium text-gray-400">{course.date}</span>
                    </div>
                ))}
            </div>
            <button className="mt-4 w-full text-indigo-600 text-sm font-semibold py-2 rounded-lg hover:bg-indigo-50 transition-colors">
                View All Activities
            </button>
        </div>
    );
...
const Home = () => {
    ...
    const stats = [
        { title: 'Learners', value: '2,114', icon: LearnersIcon, color: { bg: 'bg-indigo-600', text: 'text-indigo-600' } },
        { title: 'Courses', value: '254', icon: CourseIcon, color: { bg: 'bg-green-600', text: 'text-green-600' } },
        { title: 'Institutes', value: '32', icon: InstituteIcon, color: { bg: 'bg-red-600', text: 'text-red-600' } },
        { title: 'Recent Verification', value: '18', icon: RecentVerificationIcon, color: { bg: 'bg-blue-600', text: 'text-blue-600' } },
    ];
    ...
    return (
        <div className="space-y-8">
            ...
            {/* 4. Performance Chart and Calendar (Main Content Row) */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

                {/* Course Performance Chart (Takes 2/3 width on large screens) */}
                <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">Course performance</h3>
                    <p className="text-sm text-gray-500 mb-4">Introduction to UI Design</p>
                    {/* Placeholder for the chart */}
                    <div className="h-64 flex items-center justify-center border border-dashed border-gray-300 rounded-lg bg-gray-50">
                        <p className="text-gray-400">Chart Placeholder (e.g., using Recharts or a simple SVG)</p>
                    </div>
                </div>

                {/* Activity Calendar (Takes 1/3 width on large screens) */}
                <div className="lg:col-span-1">
                    <ActivityCalendar />
                </div>
            </div>

            {/* 5. Recent Activity List (Full width row) */}
